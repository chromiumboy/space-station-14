...
namespace Content.Client.Power.APC.UI
{
    ...
    public sealed partial class ApcMenu : FancyWindow
    {
        public ApcMenu(ApcBoundUserInterface owner)
        {
            ...
            CoverLockButton.OnPressed += _ => owner.CoverLockPressed();
        }
        ...
        public void UpdateState(BoundUserInterfaceState state)
        {
            ...
            if (CoverLockButton != null)
            {
                if (castState.HasAccess == false)
                {
                    CoverLockButton.Disabled = true;
                    CoverLockButton.ToolTip = Loc.GetString("apc-component-insufficient-access");
                }
                else
                {
                    CoverLockButton.Disabled = false;
                    CoverLockButton.ToolTip = null;
                    CoverLockButton.Pressed = castState.CoverLock;
                }

                CoverLockButton.Text = Loc.GetString(castState.CoverLock
                    ? "apc-menu-power-cell-cover-lock-engaged"
                    : "apc-menu-power-cell-cover-lock-disengaged");
            }
            ...
            if (ChargeBar != null)
            {
                ...
                if (float.IsNaN(chargePercentage))
                {
                    ChargePercentage.Text = Loc.GetString("apc-menu-power-cell-missing");
                    ChargePercentage.SetOnlyStyleClass(StyleNano.StyleClassPowerStateNone);
                }
                else
                {
                    ChargePercentage.Text = Loc.GetString("apc-menu-charge-label", ("percent", chargePercentage.ToString("P0")));
                    ChargePercentage.SetOnlyStyleClass(Label.StylePropertyFont);
                }
            }
        }

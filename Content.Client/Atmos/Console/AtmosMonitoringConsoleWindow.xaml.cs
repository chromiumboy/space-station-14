...
public sealed partial class AtmosMonitoringConsoleWindow : FancyWindow
{
    ...
    public void ShowEntites(EntityCoordinates? monitorCoords, AtmosAlarmEntry[] activeAlarms)
    {
        if (_owner == null)
            return;
        ...
        ActiveAlarms.RemoveAllChildren();

        Logger.Debug("Have " + activeAlarms.Length + " alarms active");

        foreach (var entry in activeAlarms)
        {
            MakeNewAlarmEntry(entry);
        }
    ...
    private void MakeNewAlarmEntry(AtmosAlarmEntry entry)
    {
        ...
        foreach (var alert in entry.TemperatureAlerts)
        {
            ...
            if (alert == Shared.Atmos.Monitor.AtmosMonitorThresholdBound.Upper)
